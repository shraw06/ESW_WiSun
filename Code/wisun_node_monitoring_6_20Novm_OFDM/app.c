...
static void _read_sensor_data(void);
...
#define SL_WISUN_STATUS_JSON_STR_MAX_LEN 1024
...
void app_task(void *args)
{
  ...
  app_timestamp_init();
  //Added
  sl_status_t sensor_status = sl_si70xx_init(sl_i2cspm_si7021,si7021_ADDR);
  ...
  init_app_parameters();
  app_parameters.auto_send_sec = 120;
...
static void _read_sensor_data(void)
{
  ...
  status = sl_si70xx_measure_rh_and_temp(sl_i2cspm_si7021, addr, &rh_data, &temp_data);

  if (status == SL_STATUS_OK) {
    printfBothTime("[Sensor] Temp: %.2f °C | RH: %.2f %%\n",
                   temp_data / 1000.0f, rh_data / 10.0f);
  } else {
    temp_data = -1;
    rh_data = 0;
    printfBothTime("[Failed: Sensor read failed with status 0x%04x]\n",
                   (uint16_t)status);
  }
}
