...
#ifndef SL_CATALOG_SI70XX_PRESENT
...
// --- ADD THESE LINES ---
// Check if the Si70xx sensor component is included in the project
...
// safer app_coap_reply: allocate payload copy per response
...
#include <arpa/inet.h> // <-- Add this include for inet_ntop
#ifndef INET6_ADDRSTRLEN
#define INET6_ADDRSTRLEN 46
#endif

/**
 * @brief CoAP callback for retrieving the node's own RPL information.
 * Returns a JSON object with the current RPL rank and DODAG ID.
 */
sl_wisun_coap_packet_t * coap_callback_self_rpl_info(
      const  sl_wisun_coap_packet_t *const req_packet)
{
  sl_status_t ret_rpl, ret_ip;
  sl_wisun_rpl_info_t rpl_info;
  in6_addr_t br_address;

  // Use INET6_ADDRSTRLEN for the string buffer
  char dodag_id_str[INET6_ADDRSTRLEN] = "N/A";
  uint16_t self_rank = 0xFFFF; // Default to 65535 (Infinite/Disconnected)

  // 1. Get the RPL info (for the rank)
  ret_rpl = sl_wisun_get_rpl_info(&rpl_info);

  if (ret_rpl == SL_STATUS_OK) {
    // === THIS IS THE CORRECTION ===
    // Use the 'dodag_rank' field from the struct you provided
    self_rank = rpl_info.dodag_rank;
    // === END CORRECTION ===

  } else if (ret_rpl == SL_STATUS_NOT_JOINED) {
    snprintf(dodag_id_str, INET6_ADDRSTRLEN, "Not Joined");
    // Go to the end to format the response
    goto format_response;
  } else {
    snprintf(dodag_id_str, INET6_ADDRSTRLEN, "RPL Error");
    goto format_response;
  }

  // 2. Get the DODAG ID (Border Router IP address)
  // This was missing from the rpl_info struct, so we get it here.
  ret_ip = sl_wisun_get_ip_address(SL_WISUN_IP_ADDRESS_TYPE_BORDER_ROUTER, &br_address);

  if (ret_ip == SL_STATUS_OK) {
    // Use standard inet_ntop (from arpa/inet.h) to convert the IP to a string
    inet_ntop(AF_INET6, &br_address, dodag_id_str, INET6_ADDRSTRLEN);
  } else {
    snprintf(dodag_id_str, INET6_ADDRSTRLEN, "IP Error");
  }

format_response:
  // 3. Format the response as a JSON string.
...
/**
 * @brief CoAP callback for Si7021 sensor data.
 * @param req_packet The request packet.
 * @return The response packet.
 */
sl_wisun_coap_packet_t * coap_callback_sensor_data(
    const sl_wisun_coap_packet_t *const req_packet)
{
  #define JSON_SENSOR_DATA_FORMAT_STR  \
    "{\n"                               \
    "  \"temperature_mc\": %ld,\n"     \
    "  \"humidity_mrh\": %ld\n"         \
    "}\n"

  int32_t temp_mc = 0;   // milli-Celsius
  uint32_t humidity_mrh = 0; // milli-percent RH
  sl_status_t status;

  sl_status_t sensor_status = sl_si70xx_init(sl_i2cspm_si7021, 0x40);
      if (sensor_status != SL_STATUS_OK) {
        printfBothTime("[Failed: Si7021 sensor init failed with status 0x%04x]\n", (uint16_t)sensor_status);
      } else {
        printfBothTime("Si7021 sensor initialized successfully.\n");
      }

  // Read data from the sensor.
  // Assumes the default I2C instance name for the si7021 is sl_i2c_si7021
  const uint8_t addr = 0x40;
  status = sl_si70xx_measure_rh_and_temp(sl_i2cspm_si7021, addr, &humidity_mrh, &temp_mc);

  if (status == SL_STATUS_OK) {
    // Format the JSON response with sensor data
    snprintf(coap_response, COAP_MAX_RESPONSE_LEN, JSON_SENSOR_DATA_FORMAT_STR,
             temp_mc, humidity_mrh);
  } else {
    // Report an error if the sensor read failed
    snprintf(coap_response, COAP_MAX_RESPONSE_LEN, "{\"error\":\"sensor read failed: 0x%lX\"}", status);
  }
...
/* Defensive & correct neighbor lookup using sl_wisun_get_neighbors() */
sl_wisun_coap_packet_t * coap_callback_all_statuses (
  ...
  char running_str[40] = {0};
  char connected_str[40] = {0};
  char connected_total_str[40] = {0};
  char disconnected_total_str[40] = {0};

  uint8_t rsl_in_raw_u = 0xFF;
  uint8_t rsl_out_raw_u = 0xFF;

  int rsl_in_dbm = INT_MIN, rsl_out_dbm = INT_MIN;
  int rpl_rank = -1, hopcount = -1;
  sl_status_t status;
  sl_wisun_network_info_t network_info = {0};

  /* 1) get neighbor count */
  uint8_t neighbor_count = 0;
  status = sl_wisun_get_neighbor_count(&neighbor_count);
  if (status != SL_STATUS_OK) {
    printf("DBG: sl_wisun_get_neighbor_count() failed: 0x%lx\n", status);
    neighbor_count = 0;
  } else {
    printf("DBG: neighbor_count = %u\n", neighbor_count);
  }

  /* 2) allocate array for MACs (limit to a reasonable max) */
  #define MAX_NEIGHBORS 32
  sl_wisun_mac_address_t neighbors[MAX_NEIGHBORS];
  uint8_t to_read = (neighbor_count > MAX_NEIGHBORS) ? MAX_NEIGHBORS : neighbor_count;

  if (to_read) {
    status = sl_wisun_get_neighbors(&to_read, neighbors);
    if (status != SL_STATUS_OK) {
      printf("DBG: sl_wisun_get_neighbors() failed: 0x%lx\n", status);
      to_read = 0;
    } else {
      printf("DBG: sl_wisun_get_neighbors returned %u entries\n", to_read);
    }
  }

  /* 3) try to find the parent/neighbour info by MACs */
  sl_wisun_neighbor_info_t ninfo;
  for (uint8_t i = 0; i < to_read; i++) {
    memset(&ninfo, 0, sizeof(ninfo));
    status = sl_wisun_get_neighbor_info(&neighbors[i], &ninfo);
    if (status != SL_STATUS_OK) {
      printf("DBG: sl_wisun_get_neighbor_info(mac[%u]) returned 0x%lx\n", i, status);
      continue;
    }

    /* optional: print the MAC */
    printf("DBG: neighbor[%u] mac:", i);
    for (int b = 0; b < SL_WISUN_MAC_ADDRESS_SIZE; b++) {
      printf(" %02x", neighbors[i].address[b]);
    }
    printf(" rsl_in=%u rsl_out=%u rpl_rank=%u rssi=%d\n",
           ninfo.rsl_in, ninfo.rsl_out, ninfo.rpl_rank, ninfo.rssi);

    /* Heuristic: use first neighbor that has valid rsl_in/out */
      if (ninfo.rsl_in != 0xFF) {
         rsl_in_raw_u = ninfo.rsl_in;
         rsl_in_dbm = (int)rsl_in_raw_u - 174;
     }
     if (ninfo.rsl_out != 0xFF) {
         rsl_out_raw_u = ninfo.rsl_out;
         rsl_out_dbm = (int)rsl_out_raw_u - 174;
     }
    if (ninfo.rpl_rank != 0xFFFF) {
      rpl_rank = (int)ninfo.rpl_rank;
    }

    if (rsl_in_dbm != INT_MIN || rsl_out_dbm != INT_MIN) {
      break; /* found usable data */
    }
  }

  /* 4) network info for hopcount */
  status = sl_wisun_get_network_info(&network_info);
  if (status == SL_STATUS_OK) {
    hopcount = network_info.hop_count;
    printf("DBG: hopcount = %d\n", hopcount);
  } else {
    printf("DBG: sl_wisun_get_network_info() failed: 0x%lx\n", status);
  }

  /* Normalise unknowns for JSON output */
  char rsl_in_dbm_str[16];
   char rsl_out_dbm_str[16];

   if (rsl_in_dbm == INT_MIN) {
     snprintf(rsl_in_dbm_str, sizeof(rsl_in_dbm_str), "null");
   } else {
     snprintf(rsl_in_dbm_str, sizeof(rsl_in_dbm_str), "\"%d\"", rsl_in_dbm);
   }

   if (rsl_out_dbm == INT_MIN) {
     snprintf(rsl_out_dbm_str, sizeof(rsl_out_dbm_str), "null");
   } else {
     snprintf(rsl_out_dbm_str, sizeof(rsl_out_dbm_str), "\"%d\"", rsl_out_dbm);
   }
  if (rpl_rank < 0) rpl_rank = 0;
  if (hopcount < 0) hopcount = 0;

  /* Time strings (same as your code) */
  snprintf(running_str, sizeof(running_str), dhms(now_sec()));
  snprintf(connected_str, sizeof(connected_str), dhms(now_sec() - connection_time_sec));
  snprintf(connected_total_str, sizeof(connected_total_str), dhms(connected_total_sec));
  snprintf(disconnected_total_str, sizeof(disconnected_total_str), dhms(disconnected_total_sec));

  int n = snprintf(coap_response, COAP_MAX_RESPONSE_LEN, JSON_ALL_STATUSES_FORMAT_STR,
                    running_str,
                    connected_str,
                    parent_tag,
                    (unsigned)rsl_in_raw_u,
                    (unsigned)rsl_out_raw_u,
                    rsl_in_dbm_str,
                    rsl_out_dbm_str,
                    rpl_rank,
                    hopcount,
  ...
  if (n < 0 || n >= COAP_MAX_RESPONSE_LEN) {
    snprintf(coap_response, COAP_MAX_RESPONSE_LEN, "{\"error\":\"format error\"}");
  }
...
//extern sl_wisun_mac_address_t g_parent_mac; // Reference the global variable
//
//sl_wisun_coap_packet_t * coap_callback_all_statuses (
//      const  sl_wisun_coap_packet_t *const req_packet)  {
//
//  // 1. Modify the JSON format string to include the new fields
//  //    Note: We are using numbers (%d, %u) for numeric values, not strings.
//  #define JSON_ALL_STATUSES_FORMAT_STR      \
//    "{\n"                                 \
//    "  \"running\": \"%s\",\n"            \
//    "  \"connected\": \"%s\",\n"          \
...
//
//  // 2. Declare variables for new stats
//  sl_wisun_rpl_info_t rpl_info = {0};
//  sl_wisun_neighbor_info_t parent_info = {0};
//  sl_status_t status;
//
//  // Initialize with "unknown" or default values
//  uint16_t rpl_rank = 0xFFFF; // 65535, often used for "unknown"
//  uint16_t hopcount = 0xFFFF;
//  int16_t rsl_in_dbm = -174; // RSL min value
//  int16_t rsl_out_dbm = -174; // RSL min value
//
//
...
//
//  // 4. Get Node RPL info (Rank and Hop Count)
//  status = sl_wisun_get_rpl_info(&rpl_info);
//  if (status == SL_STATUS_OK) {
//    rpl_rank = rpl_info.dodag_rank;
//    // Calculate hop count from rank and min_hop_rank_increase
//    if (rpl_info.min_hop_rank_increase > 0) {
//      hopcount = (uint16_t)(rpl_info.dodag_rank / rpl_info.min_hop_rank_increase);
//    }
//  }
//
//  // 5. Get Parent RSL info (Requires parent's MAC address)
//  //    This assumes g_parent_mac is set during the connection event
//  status = sl_wisun_get_neighbor_info(&g_parent_mac, &parent_info);
//  if (status == SL_STATUS_OK) {
//    // RSL values are an 8-bit value (0-254) offset by -174.
//    // 0xFF (255) means unknown.
//    if (parent_info.rsl_in != 0xFF) {
//      rsl_in_dbm = (int16_t)parent_info.rsl_in - 174;
//    }
//    if (parent_info.rsl_out != 0xFF) {
//      rsl_out_dbm = (int16_t)parent_info.rsl_out - 174;
//    }
//  }
//
//  // 6. Format the complete JSON string
...
uint8_t app_coap_resources_init() {
  ...
  coap_resource.data.uri_path = "/status/self/rpl_info";
    coap_resource.data.resource_type = "json";
    coap_resource.data.interface = "node";
    coap_resource.auto_response = coap_callback_self_rpl_info;
  ...
  coap_resource.data.uri_path = "/sensor/si7021"; // The new URI
  coap_resource.data.resource_type = "json";
  coap_resource.data.interface = "sensor";
  coap_resource.auto_response = coap_callback_sensor_data; // The new callback
  coap_resource.discoverable = true;
  assert(sl_wisun_coap_rhnd_resource_add(&coap_resource) == SL_STATUS_OK);
  count++;
  ...
  printf("[COAP-DEBUG] Registered resource: %s\n", coap_resource.data.uri_path);
